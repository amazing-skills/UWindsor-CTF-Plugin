...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		getCommand("ctf").setExecutor(new CommandManager());
		...
		wipeScoreboard(); // Scoreboards save through bukkit, need to wipe and re-add them on reload/start
							// so they're in sync with plugin
		constructScoreboard();
	...
	public void loadConfiguration() {
		...
		getConfig().options().copyDefaults(true); // NOTE: You do not have to use "plugin." if the class extends the
													// java plugin
		...
		if (getConfig().getConfigurationSection("Teams") != null) {
			for (String team : getConfig().getConfigurationSection("Teams").getKeys(false)) {
				...
				TeamManager.addTeam(getConfig().getString(path + ".name"), getConfig().getString(path + ".color")); // Add
																													// the
																													// team
	...
	public void constructScoreboard() {
		...
		Objective objective = board.registerNewObjective("Scoreboard", Criteria.DUMMY,
				...
				for (Player player : Bukkit.getServer().getOnlinePlayers()) {
					...
					for (Team team : TeamManager.getTeams()) {
						if (TeamManager.getPlayerTeam(player) != null
								&& !(TeamManager.getPlayerTeam(player).equals(team))
								&& !Safezones.list.contains(team)) {
